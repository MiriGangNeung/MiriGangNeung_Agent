...
배경 변형 판정이 결과를 통째로 버리던 것을 경고로 바꿨다. 실측에서 경포해수욕장
합성이 **얼굴 유사도 0.483으로 통과했는데도** 이 사유 하나로 `FAILED`가 됐고,
사용자는 32초와 약 $0.067을 쓰고 빈손으로 끝났다. 결과 이미지는 저장 전에 버려져
얼마나 변형됐는지 확인할 방법조차 없었다.

근거: 생성 모델은 배경 픽셀을 복사하는 게 아니라 다시 그리므로 어느 정도 변형은
필연이고, "얼마나 달라져야 변형인가"의 기준은 검사기가 쥐고 있다. 배경이 실제와
다를 수 있다는 것은 **합성 요청 단계에서 미리 안내**하고, 결과는 경고와 함께 준다.

`WARNING_REASON_CODES`에 코드만 추가하면 `warning_for()`가 `DEFAULT_REASON_MESSAGE`
("품질·안전성 기준을 통과하지 못했습니다")로 폴백해, 사진을 받은 사용자에게 거부
문구가 붙는 모순이 생긴다. 경고 전용 문구를 함께 넣었다.

### 주요 변경 파일

- `app/pipeline/safety.py` — `WARNING_REASON_CODES`에 `BACKGROUND_ALTERED` 추가,
  `WARNING_MESSAGES`에 경고 문구 추가
- `docs/AI_API_CONTRACT.md` — 거부 목록에서 경고 절로 이동. **백엔드 팀 전달 필요**
  (API 필드는 그대로. `GET /v1/meta`의 `safetyReasonCodes[].severity`가
  `reject` → `warn`으로 바뀐다)
- 테스트 2건을 새 동작에 맞게 수정

프론트(별도 레포)는 합성 요청 화면에 "배경이 원본 사진과 다소 다르게 표현됐을 수
있고…" 안내를 넣고, 결과 화면에서 사진을 다시 고르지 않고도 재생성할 수 있게 했다.

### 테스트 결과

`pytest -q` **200 통과**. `GET /v1/meta`에서 `BACKGROUND_ALTERED`의 `severity`가
`warn`으로 나가는 것을 실서버로 확인했다.

Gemini 실호출 1건(경포해수욕장, 유사도 0.590, 31초)은 **검사기를 그냥 통과해 경고
경로를 타지 않았다.** 생성이 확률적이라 `BACKGROUND_ALTERED`를 의도적으로 발생시킬
수단이 없다(얼굴 경고와 달리 임계값 설정이 없고 Gemini 판정에 달렸다). 따라서 경고
전달 경로는 단위 테스트와 `/v1/meta` severity로만 검증했고, 실 판정으로는 재현하지
못했다.

### 다음 담당자에게

거부로 되돌리려면 `WARNING_REASON_CODES`에서 코드를 빼면 되지만, 그때는 프론트의
요청 단계 안내 문구도 함께 손봐야 앞뒤가 맞는다.

---

## 2026-09-09 (2) — 사진 수 기준을 리포 데이터로 통일 (claude)
...
노션 리포트는 노출 가능 사진을 64장으로 적었는데 리포의 `place_insights.json`에
노출 규칙을 적용하면 54장이다. 장소 목록(23곳)은 양쪽이 정확히 일치하고 사진 수만
달랐다. **리포 데이터를 기준으로 통일했다** — 서비스가 실제로 노출하는 것은 커밋된
JSON에서 계산한 값이고, 코드와 문서가 어긋나면 코드가 옳다(AGENTS.md §11).

차이는 두 겹이었다.

1. **64 → 58**: 커밋된 JSON의 `portraitViability != low`가 58장. 노션 생성 이후 판정이
   갱신되며 6장이 low로 내려갔다.
2. **58 → 54**: ADR-0007이 정한 나머지 두 규칙(드론 항공샷 제외, `standableSurface`
   존재)이 4장을 더 걸러낸다. 노션 리포트는 이 둘을 규칙으로 명시하지 않고
   `portraitViability` 하나로만 설명했다.

장소별로는 7곳이 줄었다: 국립대관령치유의숲 5→2, 구룡폭포(소금강) 3→1,
강릉 경포대 5→4, 정동심곡 바다부채길 4→3, 정동진 4→3, 정동진해변 2→1, 솔바람다리 2→1.
등급 분포는 추천 20→14, 보통 44→40.

### 주요 변경 파일

- `docs/adr/0007-place-exposure-single-source.md` — "사진 수의 기준" 절 신설
- 노션 「배경 사진 VLM 사전 분석 리포트」 — 요약 수치(23곳/54장), 판정 분포,
  노출 가능 장소 표 23행을 리포 데이터로 교체하고 정렬을 복구
- 노션 「배경 사진 속성 데이터셋」 — 요약 수치에 스냅샷임을 명시하고, 단일 출처가
  `viable_places.json`임을 적음. 64행 레코드 표 자체는 원본 기록으로 보존
- 코드 변경 없음. `viable_places.json`은 이미 54장으로 생성되고 있었다

### 테스트 결과

`pytest -q` 200 통과. `scripts/export_place_filter.py --check` 통과 (23곳 / 54장).
코드 변경이 없어 재생성 결과도 동일하다.

### 다음 담당자에게

노션 데이터셋 페이지의 64행 표는 **작성 시점 스냅샷**이다. 지우지 않고 남겼으니,
현재 노출 목록이 필요하면 그 표가 아니라 `viable_places.json`을 보면 된다.
